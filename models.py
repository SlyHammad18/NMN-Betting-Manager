...
class Game(db.Model):
    ...
    teams = db.relationship('Team', backref='game', cascade="all, delete-orphan", lazy=True)
    rounds = db.relationship('Round', backref='game', cascade="all, delete-orphan", lazy=True)
...
class Team(db.Model):
    ...
    bets = db.relationship('Bet', backref='team', cascade="all, delete-orphan", lazy=True)
...
class Round(db.Model):
    ...
    previous_carryover = db.Column(db.Float, default=0.0)
    created_at = db.Column(db.DateTime, default=datetime.utcnow)
    bets = db.relationship('Bet', backref='round', cascade="all, delete-orphan", lazy=True)
